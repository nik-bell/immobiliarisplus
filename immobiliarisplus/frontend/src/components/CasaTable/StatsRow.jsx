/**
 * @file StatsRow.jsx
 * @description Statistics row with status filters.
 * 
 * Displays valuation counts by status with clickable filter chips.
 * Includes refresh button and filters available statuses by user role.
 * 
 * @module components/CasaTable/StatsRow
 */
...
/**
 * Dashboard statistics and filter row.
 * 
 * Shows valuation counts grouped by status with interactive filter chips.
 * Agents cannot see the "Non assegnati" filter (admin only).
 * Includes a refresh button to reload dashboard data.
 * 
 * @returns {JSX.Element} Stats row with filters and refresh button
 */
export default function StatsRow() {
  ...
  // Count by status (using rawCases for global numbers)
  const counts = {
  ...
  // Filter out "non_assegnati" for agents
